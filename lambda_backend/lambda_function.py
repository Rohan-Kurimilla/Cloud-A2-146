...
def build_music_id(item):
    return f"{item.get('artist')}#{item.get('title')}#{item.get('year')}#{item.get('album')}"


def handle_home():
    return response(200, {
        "message": "AWS Music Subscription Lambda Backend is running"
    })
...
def handle_get_subscriptions(event):
    ...
    subscriptions = [add_image_url(item) for item in subscriptions]
...
def handle_delete_subscription(event):
    ...
    if not email or not music_id:
        return response(400, {
            "success": False,
            "message": "email and music_id are required"
        })
